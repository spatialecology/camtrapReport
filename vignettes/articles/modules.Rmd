---
title: "Module Management and Extension"
...
vignette: >
  %\VignetteIndexEntry{Module Management and Extension}
...
`install_all()` scans the `#| packages:` options of all bundled modules and all
registered user-managed module directories. It installs only dependencies that
are not already available. Because installation is delegated to `pak`, `pak`
must be installed before `install_all()` is called.

```{r install-module-dependencies, eval=FALSE}
# Install pak once if it is not already available
install.packages("pak")

# Discover and install missing dependencies declared by the modules
install_all()
```

For a package that is available only from GitHub or another non-CRAN source,
provide a named `pak` package reference. The name must match the package name
declared in the module:

```{r install-module-github-dependency, eval=FALSE}
install_all(
  package_references = c(myPackage = "owner/myPackage")
)
```
