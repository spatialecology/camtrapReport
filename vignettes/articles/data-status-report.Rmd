...
status(
  cm,
  filename = file.path(output_dir, "data_status"),
  view = TRUE
)
```
...
The function also invisibly returns the path to the HTML report. Supply `filename` to choose a deliberate file stem or an existing output path. The examples use `tempdir()` to avoid writing to the working directory:

```{r data-status-output-path, eval=FALSE}
status_file <- status(
  cm,
  filename = file.path(tempdir(), "data_status_2026"),
  view = FALSE
)
```
