...
test_that("low-level report chunks and Pandoc attributes are preserved", {
  ...
  expect_identical(.clean_chunk_name("bad & name"), "bad_name")
  expect_identical(.clean_chunk_name("", ""), "module")
  expect_identical(.glueRchunk(NULL), "")
  expect_error(
    .safe_glue_text("{missing_value}", env, "bad_section"),
    "bad_section"
  )
})
...
test_that("quick section checks report rendering success and failure", {
  ...
  expect_true(.QuickTestReportSection(valid))

  testthat::local_mocked_bindings(
    render = function(...) stop("intentional render failure"),
    .package = "rmarkdown"
  )
  expect_false(.QuickTestReportSection(valid))
})
