test_that("module tree helpers preserve parent-child ordering", {
  ...
  expect_identical(ct_internal(".subtree_end")(info, "methods"), 2L)
  expect_identical(ct_internal(".get_descendants")(info, "methods"), "sampling")
  expect_identical(
  ...
  inserted <- ct_internal(".insert_module_info")(
    info,
    name = "effort",
    parent = "methods",
    after = "sampling"
  ...
  before <- ct_internal(".insert_module_info")(
    inserted,
    name = "locations",
    parent = "methods",
    before = "sampling"
...
test_that("modules can be added, deleted, restored, and purged in a temporary library", {
  ...
  parent_added <- ct_internal(".add_Module")(
    parent_file,
    after = "captures",
    dir = module_dir,
    test = FALSE
  )
  child_added <- ct_internal(".add_Module")(
    child_file,
    dir = module_dir,
    test = FALSE
  )
  ...
  listed <- ct_internal(".list_Modules")(dir = module_dir, validate = TRUE)
  located <- ct_internal(".locate_Module")(
    c("test_parent", basename(child_added$file)),
    dir = module_dir
  )
  audit <- ct_internal(".audit_Modules")(dir = module_dir, validate = TRUE)
  ...
  deleted <- ct_internal(".delete_Module")(
    "test_parent",
    recursive = TRUE,
    dir = module_dir
  )
  ...
  expect_identical(nrow(ct_internal(".list_Trash")(dir = module_dir)), 2L)

  restored <- ct_internal(".recover_Module")(
  ...
  expect_identical(nrow(ct_internal(".list_Trash")(dir = module_dir)), 0L)

  remaining_index <- ct_internal(".purge_Trash")(
    recovered_only = TRUE,
    dir = module_dir
  )
...
test_that("module inventory reports invalid and duplicate YAML files", {
  ...
  intro <- ct_internal(".find_module_file")(module_dir, "introduction")

  writeLines(c("---", "name: [not valid"), invalid)
  file.copy(intro, duplicate)

  inventory <- ct_internal(".module_inventory")(
    dir = module_dir,
    include_trash = TRUE,
    validate = TRUE
  )
  listed <- ct_internal(".list_Modules")(
    dir = module_dir,
    include_invalid = TRUE,
    validate = TRUE
  )
  validation <- ct_internal(".validate_module")(invalid)
...
test_that("empty module directories get a usable module index", {
  ...
  inventory <- ct_internal(".module_inventory")(dir = module_dir)
  trash <- ct_internal(".read_trash_index")(module_dir)

  expect_identical(info$name, c("introduction", "results"))
  expect_identical(nrow(inventory), 0L)
  expect_identical(nrow(trash), 0L)
  expect_error(ct_internal(".modules_info_path")(tempdir()), "Could not find")
})
