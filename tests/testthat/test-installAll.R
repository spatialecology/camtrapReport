test_that("installer inventory helpers return configured package vectors", {
  ...
  expect_false(camtrapReport:::.is.installed("a_package_that_does_not_exist_123"))
  expect_true(camtrapReport:::.loadLib(list(c("methods", "stats"))))
})

test_that("install_All reports an already satisfied mocked inventory", {
  local_mocked_bindings(
    .getPackageList = function() character(),
    .getPackageGitHubList = function() stats::setNames(character(), character()),
...
test_that("install_All exercises missing and update paths without installing", {
  installed <- function(x) stats::setNames(rep(FALSE, length(x)), x)

  local_mocked_bindings(
    .getPackageList = function() "mockCranPackage",
    .getPackageGitHubList = function() c(mockGitHubPackage = "owner/repository"),
    .is.installed = installed,
    install.packages = function(...) TRUE,
    remove.packages = function(...) TRUE,
