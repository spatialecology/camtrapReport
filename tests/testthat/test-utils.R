test_that("formatting helpers handle normal and boundary inputs", {
  paste_list <- ct_internal(".paste_comma_and")
  format_duration <- ct_internal(".format_duration")
  format_size <- ct_internal(".format_file_size")
...
test_that("text parsing helpers preserve their documented contracts", {
  trim_one <- ct_internal(".trim")
  trim_many <- ct_internal(".trim_chr")
  chunk_name <- ct_internal(".extract_chunk_name")
  ...
  expect_identical(ct_internal(".rmChar")("abcdef", c(1, 2), TRUE), "cde")
  expect_identical(ct_internal(".firstUpper")(c("FOX", NA)), c("Fox", ""))
  expect_identical(ct_internal(".pretty_label")(c("wild_mammals", "birds")),
                   "wild mammals and birds")
})

test_that("date, time, and filename helpers cover common input forms", {
  get_year <- ct_internal(".getYear")
  get_hour <- ct_internal(".get_hour")
  time_length <- ct_internal(".get_Time_length")
  ...
  expect_true(ct_internal(".isZip")("DATA.ZIP"))
  expect_true(ct_internal(".isJson")("data.Json"))
  expect_false(ct_internal(".isZip")(NULL))
  expect_true(ct_internal(".is.POSIXct")(as.POSIXct("2024-01-01", tz = "UTC")))
  expect_identical(ct_internal(".getFormat")("2024-01-01T13:30:00"),
                   "%Y-%m-%dT%H:%M:%OS")
  expect_true(is.na(ct_internal(".getFormat")("not a date")))
})

test_that("small data helpers return stable base objects", {
  bind_rows <- ct_internal(".bind_rows")
  get_match <- ct_internal(".get_match")
...
test_that("file and size helpers inspect temporary data without side effects", {
  ...
  info <- ct_internal(".file_info")(file.path(d, "sample.data.csv"))
  ...
  expect_identical(ct_internal(".file_info")("README")$extension, NA_character_)
  expect_identical(ct_internal(".estimate_camdata_size")("missing")$size_class,
                   "unknown")
})
...
test_that("evaluation and package helpers are safe for core packages", {
  ...
  expect_false(ct_internal(".loadPKG")("a_package_that_does_not_exist_123"))
  expect_identical(ct_internal(".suppress_startup")({
    message("suppressed message")
    7
  }), 7)
  expect_identical(ct_internal(".make_safe_module_code")(NULL), "")
  expect_identical(
    ct_internal(".make_safe_module_code")(c("x <- 1", "x + 1")),
    "x <- 1\nx + 1"
  )
})
...
test_that("nested section and empty taxonomy helpers return stable structures", {
  ...
  expect_true(is.na(ct_internal(".findParent")(tree, "unknown")))
  expect_true(is.na(ct_internal(".findParent")(list(), "parent")))

  ncbi <- ct_internal(".getMissingTaxon_NCBI")(character())
  gbif <- ct_internal(".getMissingTaxon_GBIF")(character())
...
test_that("spatial helpers recognise geographic and projected terra objects", {
  ...
  projected <- ct_internal(".get_projected_vect")(geographic)
  
  expect_false(ct_internal(".is.projected")(geographic))
  expect_true(ct_internal(".is.projected")(projected))
  expect_s4_class(projected, "SpatVector")
  expect_identical(ct_internal(".get_projected_vect")(projected), projected)
})
...
test_that("sf study-area boundaries are projected automatically", {
  ...
  projected <- ct_internal(".get_projected_sf")(x)
...
test_that("the base correlation plot draws on a non-interactive device", {
  ...
  expect_type(ct_internal(".basic_corrplot")(x), "list")
})
