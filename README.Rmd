...
Install `camtrapReport` from GitHub:
...
## Install optional module dependencies

`camtrapReport` keeps packages required by individual report modules optional.
To enable all modules, `install_all()` automatically discovers and installs
...
Dependencies that are already installed are retained. The function also scans
dependencies declared by registered user-defined modules.
...
```{r generate-data-status, message=FALSE, warning=FALSE, results='hide'}
status_file <- status(
  cm,
  view = FALSE
)
...
```{r generate-ecological-report, message=FALSE, warning=FALSE, results='hide'}
report_file <- report(
  cm,
  view = FALSE
)
...
the Research Institute for Nature and Forest (INBO) through GBIF. The subset is
intended for demonstration and testing, not ecological inference. Its preparation is documented [here](https://github.com/spatialecology/camtrapReport/blob/main/inst/external/README-Leuven-dataset.md).
