...
.insert_module_info <- function(
  ...
  if (name %in% info$name) {
    stop("Module already exists in .info: ", name)
  }
  ...
  if (!is.null(before)) {
    ...
    if (!after %in% info$name) {
      stop("'after' was not found in .info: ", after)
    }
...
.add_Module <- function(
  ...
  if (isTRUE(test)) {
    ...
    if (!isTRUE(vv)) {
      if (is.null(object)) {
        stop(
          "Testing of the module caused error...(does the test require the ",
          "camReport object?!)"
        )
      } else {
  ...
  active_names <- unique(inv$module_name[
  ...
  if (!ok) {
    stop("Could not copy module file to: ", dest)
  }
...
.delete_Module <- function(
  ...
  if (length(x) == 0L) {
    stop("No module names were supplied.")
  }
...
.recover_Module <- function(
  ...
  for (i in seq_len(nrow(rec))) {
    ...
    w <- which(
      trash_idx$batch_id == row$batch_id &
        trash_idx$name == nm &
...
.list_Modules <- function(
  ...
  matched_status <- ifelse(out$valid[hit], "ok", "invalid_yml")
  matched_status[out$duplicate_module_name[hit]] <- "duplicate_module_name"
  out$status[hit] <- matched_status
...
.audit_Modules <- function(
  ...
  list(
    ...
    duplicate_module_names = unique(valid_named$module_name[
...
.scan_module_files_from_dir <- function(
  module_dir,
  include_trash = FALSE,
  validate = FALSE
) {
  ...
  dup <- duplicated(out$module_name) |
...
.locate_Module <- function(
  ...
  inv[
    inv$module_name %in% x | inv$filename %in% x | inv$file_stem %in% x,
    ,
    drop = FALSE
  ]
}
...
#'
#' Because modules can execute arbitrary R code with the permissions of the
#' current R process, only install modules from trusted sources and inspect
#' their contents before running them.
#'
#' `add_Module()` adds a new YAML-defined module to the package module
#' directory. The module can optionally be inserted before or after an existing
#' module.
#'
...
#' @aliases move_Module,character-method remove_Module,character-method
#' @aliases empty_trash,ANY-method restore_Module,character-method
