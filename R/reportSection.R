# Functions for creating and rendering camtrapReport report sections
# Licence: MIT
#--------
...
#' section of a [`camReport`][camReport-classes] report. Report sections can be
...
#'
#' This function is useful for creating custom report modules or extending the
#' default camtrapReport workflow with project-specific text, tables, figures,
#' or analyses.
#'
#' @param name A character string giving a unique name for the report section.
#' @param title A character string giving the section title displayed in the
#'   report. The default is `""`.
#' @param parent A character string naming the parent section, or `NULL`
#'   (default) for a top-level section.
#' @param txt A character vector or list containing text to include in the
#'   report section. The default is `NULL`.
#' @param code_setting Optional R Markdown chunk options for the code chunk. For
#'   example, `{c(echo = FALSE, results = "asis")}`. The default is `NULL`.
#' @param packages An optional character vector giving packages required by the
#'   code chunk. The default is `NULL`.
#' @param code Optional R code supplied inside braces, for example,
#'   `{ plot(1:10) }`.
#'
#' @return A `.textSection` object.
#'
#' @seealso [camData()], [report()], [updateReportSection()], [testSection()]
#' @family report sections
#'
#' @usage reportSection(name, title, parent, txt, code_setting, packages, code)
#' @rdname reportSection
#' @aliases reportSection
#'
#' @examples
#' tx <- reportSection(
#'   name = "introduction",
