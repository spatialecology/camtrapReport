...
.getSequences <- function(media) {
  ...
  # summarize per key
  sequences <- .eval(
    "sequences[, list(deploymentID = unique(deploymentID),
...
.get_Taxonomic_DF <- function(x) {
  ...
  if (all(w == 0)) {
    ...
    if (length(unique(.tmp[!is.na(.tmp)])) > .w) {
      ...
      for (i in seq_len(.w)) {
        vernacular_name <- vapply(
          x,
          function(z) names(z$vernacularNames)[i],
          character(1)
        )
        n[i] <- names(sort(table(vernacular_name), decreasing = TRUE))[1]
      }
    } else {
      ww <- which.max(w)
...
setMethod(
  ...
  function(data, habitat, study_area = NULL, update = FALSE, ...) {
    ...
    cm$description <- paste0(
      ...
      cm$siteName,
      ", located in ",
      cm$data_status$Spatial$country,
      ". ",
      ...
      cm$data_status$Species$Keep_sp_n,
      " species recorded. ",
