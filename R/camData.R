...
.addReportSections <- function(cm) {
  ...
  .txx <- .getTextObj(name='introduction',title='Introduction',
                      txt=list(p1="Terrestrial wildlife populations are fundamental components of ecosystem structure and function. Across taxonomic groups, wildlife contributes to key ecological processes, including nutrient cycling, energy flow, seed dispersal, trophic interactions, and habitat modification, thereby supporting biodiversity and enhancing ecosystem resilience (Dirzo et al., 2014). However, wildlife populations are increasingly exposed to environmental and anthropogenic pressures, such as habitat alteration, human disturbance, and climate change, which may affect their distribution, abundance, and behaviour across space and time (Newbold et al., 2015). Quantifying these responses is therefore essential for effective protected-area management and evidence-based conservation (Coad et al., 2015).",
                               p2 = "Camera trapping has become an affordable, reliable, and non-invasive method for monitoring wildlife populations (Burton et al., 2015). By automatically recording images or videos when animals trigger infrared sensors, camera traps generate valuable information on habitat use, activity patterns, species occurrence, and relative abundance while minimizing disturbance to wildlife. These data are widely used in ecological research, biodiversity monitoring, and adaptive conservation management (Burton et al., 2015).",
                               p3 = "Here, we report on camera-trap surveys conducted in {siteName}, {country}. The aim of this study is to estimate and monitor {focus_groups} over time and across deployments within the site, and to improve understanding of ecological patterns in spatial and temporal variation in occupancy and behaviour."
                      ))
