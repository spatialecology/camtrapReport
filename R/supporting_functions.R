...
.calc_effort <- function(
  ...
  dep <- .eval(
    'dep |>
    dplyr::mutate(
      effort_seconds = as.numeric(difftime(
        .data[["deploymentEnd"]],
...
#---------------
# A customised version of merge_tibbles from ctdp, developed by Henjo de
# Knegt.
...
#-----------
# x: list of camera-trap data.frames
# only_species = TRUE excludes higher taxa and retains species-level names.
...
.captures <- function(
  ...
  if (is.null(by)) {
    effort_value <- if (
      "effort" %in% names(effort_tbl) && nrow(effort_tbl) > 0
    ) {
      effort_tbl[["effort"]][1]
    } else {
      NA_real_
    ...
    effort_day_value <- if (
      "effort_days" %in% names(effort_day) && nrow(effort_day) > 0
    ) {
  ...
  if ("individuals" %in% names(cap)) {
    ...
    cap[["individual_rai"]] <- rate_multiplier *
      cap[["individuals"]] /
      cap[["effort_days"]]
  }
...
.fit_detmodel <- function(
  ...
  mod <- suppressWarnings(suppressMessages(.eval(
...
.fit_actmodel <- function(
  ...
  if (nrow(obs) > 1) {
    ...
    # Solar time is evaluated in the local analysis environment.
    obs$solartime <- .eval(
      'with(obs,
          activity::wrap(
            activity::solartime(
    ...
    .eval(
      paste0(
        'activity::fitact(obs$solartime, adj = 1.5, ',
        'sample = "data", reps = reps)'
      ),
      environment()
    )
  } else {
...
.read_section_module <- function(path) {
  ...
  if (
    !is.null(m$parent) && identical(tolower(as.character(m$parent)), "null")
  ) {
    m$parent <- NULL
  }
...
# Read setting lines such as setting, name, and packages from the code.
...
.read_yml <- function(x) {
  ...
  if (length(code_fields) == 1 && is.null(f[[code_fields]])) {
    ...
    .w <- code_fields
    for (i in .w) {
      code <- .parse_setting_lines(
        f[[i]],
        key = c('name', 'packages', 'setting')
      )
      #---
      if (!is.null(code)) {
        if (is.null(code$setting$name)) {
          code$setting$name <- paste0(f$name, '__code')
        } else {
          code$setting$name <- paste0(f$name, '__', code$setting$name)
        }
        #---
        if (!is.null(code$setting$setting)) {
          code$setting$setting <- toString(code$setting$setting)
        } else {
          code$setting$setting <- NULL
        }
...
.left_join <- function(d1, d2, by) {
  if (length(by) == 1) {
    if (!by %in% colnames(d1) && by %in% colnames(d2)) {
      stop('the "by" column does not exist in both data!')
    }
    merge(d1, d2, by = by, all.x = TRUE)
  } else if (length(by) == 2) {
    if (!by[1] %in% colnames(d1) && by[2] %in% colnames(d2)) {
...
.attach_status_modules <- function(cm, n = 'all') {
  ...
  if (is.character(n)) {
    if ('all' %in% n) {
      n <- cm$reportObjectElements$Status_modules_info$name
    } else {
      nn <- cm$reportObjectElements$Status_modules_info$name
      n <- nn[nn %in% n]

      if (length(n) == 0) {
        stop('No data status modules (status_report sections) are selected!')
      }
    }
  }
  #------
